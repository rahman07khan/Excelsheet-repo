...
def processTestFolder(directory, test_name):
    ...
    time_data = defaultdict(lambda: defaultdict(lambda: {"Starttime": None, "Endtime": None}))  # Store Start and End Times
            ...
            for line in lines:
                ...
                for part in parts[1:]:  
                    ...
                    if key == "Transtype":
                        ...
                        time_data[master_name][window]["Starttime"] = float(value)
                    elif key == "Endtime":
                        time_data[master_name][window]["Endtime"] = float(value)
    ...
    with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
        ...
        # Create Master Sheets (Each master with READ/WRITE counts and Start/End Times) & Sort Windows
        for master, window_data in transaction_counts.items():
