    ...
    try:
        with open(tempFile, "r") as filePtr, open(UpdateFile, "w") as filePtr_Write:
            ...
            for line in filePtr:
                ...
                req_urgency_match = re.search(r'rEQFLIT_QOS:(\d+)', line)
                ...
                try:
                    ...
                    req_urgency_value = req_urgency_match.group(1)
                ...
                if NthWindow != PrevNthWindow:
                    ...
                    filePtr_Write.write(f"Window{PrevNthWindow}={WindowBW},Starttime={MinStartTime},Endtime={MaxEndTime},Transtype={transtype},Opcode={opcode_val},REQ_Urgency={req_urgency_value},RES_Urgency={res_urgency_value}\n")
    ...
    try:
        with open(tempFile, "r") as filePtr, open(UpdateFile, "w") as filePtr_Write:
            ...
            for line in filePtr:
                ...
                req_urgency_match = re.search(r'urgency=([^, ]+)', line)
                ...
                if NthWindow != PrevNthWindow:
                    WindowByteCount = PrevTransactionByteCount - PrevWindowByteCount
                    WindowBW = WindowByteCount / Window
                    filePtr_Write.write(f"Window{PrevNthWindow}={WindowBW},Starttime={MinStartTime},Endtime={MaxEndTime},Transtype={trans_type},Opcode={opcode_val},REQ_Urgency={req_urgency_value},RES_Urgency={res_urgency_value}\n")
                    PrevNthWindow = NthWindow
                    PrevWindowByteCount = PrevTransactionByteCount
    ...
    try:
        with open(tempFile, "r") as filePtr, open(UpdateFile, "w") as filePtr_Write:
            ...
            for line in filePtr:
                ...
                master_match = re.search(r"master=([A-Za-z])", line)
                transtype_match = re.search(r"trans_type=(\w+)", line)
                req_urgency_match = re.search(r'arqos=([^, ]+)', line)
                res_urgency_match = re.search(r'awqos=([^, ]+)', line) 
                if not start_match or not end_match:
                    print(f"Skipping line (missing start/end time): {line.strip()}")
                    continue  # Skip invalid lines
                ...
                if NthWindow != PrevNthWindow:
                    WindowByteCount = PrevTransactionByteCount - PrevWindowByteCount
                    WindowBW = WindowByteCount / Window
                    filePtr_Write.write(f"Window{PrevNthWindow}={WindowBW},Starttime={MinStartTime},Endtime={MaxEndTime},Transtype={trans_type},REQ_Urgency={req_urgency_value},RES_Urgency={res_urgency_value}\n")
                    PrevNthWindow = NthWindow
                    PrevWindowByteCount = PrevTransactionByteCount
                PrevTransactionByteCount = CumlByteCount
    except Exception as e:
